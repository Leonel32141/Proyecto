...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 14-09-2026 a las 14:59:59
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_general_actualizar` (IN `p_sql` TEXT)   BEGIN
    SET @query = p_sql;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_general_eliminar` (IN `p_sql` TEXT)   BEGIN
    SET @query = p_sql;
    PREPARE stmt FROM @query;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_general_insertar` (IN `p_sql` TEXT)   BEGIN
    SET @query = p_sql;
    PREPARE stmt FROM @query;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_general_obtener_por_id` (IN `p_sql` TEXT)   BEGIN
    SET @query = p_sql;
    PREPARE stmt FROM @query;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_general_obtener_todos` (IN `p_sql` TEXT)   BEGIN
    SET @query = p_sql;
    PREPARE stmt FROM @query;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
